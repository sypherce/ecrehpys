...
async function init(
	...
	let api_client_token = new apiClient({ authProvider: streamer_auth_provider });

	const rewards = await twurple.getCustomRewards();
	rewards.forEach((element) => {
		if (element.title !== 'Curse the Run' && element.title !== 'Bless the Run' && /^[0-9]+.*!$/i.test(element.title) === false)
	...
	listener.onChannelRedemptionAdd(broadcaster_id, (event) => {
		...
		if (event.rewardTitle !== 'Bless the Run' && event.rewardTitle !== 'Curse the Run' && event.rewardTitle !== `${thize(1)}!`)
	...
	first = fs.existsSync('config/first.json') ? JSON.parse(fs.readFileSync('config/first.json'))[0].first : 100;
	if (first < 1) first = 1;
