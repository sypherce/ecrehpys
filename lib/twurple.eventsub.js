...
async function init(
	...
	first_reward = (async () => {
		...
		if (typeof first_reward === 'undefined') first_reward = await twurple.createCustomReward(data);
		('use strict');
		...
		async function resetFirst() {
			...
			fs.writeFileSync(
				'first.json',
				prettyStringify([{ first }], { indent: '\t', maxLength: 1000, maxNesting: 2 })
			);
			twurple.updateCustomReward(first_reward.id, {
				cost: first, //The channel points cost of the reward.
				prompt: `You've done it, you're ${thize(first)}!`, //The prompt shown to users when redeeming the reward.
		...
		async function init(
			...
			//setup rewards
			first_reward = async () => {
				first = fs.existsSync('first.json') ? JSON.parse(fs.readFileSync('first.json'))[0].first : 1;
				if (first < 1) first = 1;
				...
				const data = {
					...
					prompt: `You've done it, you're ${thize(first)}!`,
					title: `${thize(first)}!`,
					userInputRequired: false,
				};
				const list = await twurple.getCustomRewards();
				const find_nth_reward = list.find((element) => /^[0-9]+.*!$/i.test(element.title));
				if (typeof find_nth_reward === 'undefined') {
					return await twurple.createCustomReward(data);
				} else {
					return await twurple.updateCustomReward(first_reward.id, data);
				}
			};
