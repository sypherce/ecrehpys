...
async function init(streamer_user = process.env.STREAMER_USER, streamer_id = process.env.STREAMER_ID, streamer_oauth = process.env.STREAMER_OAUTH, listener_secret = process.env.LISTENER_SECRET) {
	...
	let api_client2 = new apiClient({ authProvider: streamer_auth_provider });
	...
	first = fs.existsSync('first.json') ? JSON.parse(fs.readFileSync('first.json')).at(0).first : 100;
	if(first < 1)
		first = 1;
	...
	listener.onChannelSubscription(broadcaster_id, (event) => {
		server.sendMessage('Audio', 'assets/alerts/alert_sub.mp3');
		server.sayWrapper(`${event.userDisplayName} just subbed!`)
	});
	listener.onChannelSubscriptionGift(broadcaster_id, (event) => {
		server.sendMessage('Audio', 'assets/alerts/alert_sub.mp3');
		const gifterDisplayName = (!event.isAnonymous || event.gifterDisplayName !== 'unoridnarywarlord') ?
			event.gifterDisplayName : anonymous;
		server.sayWrapper(`${gifterDisplayName} just gifted a sub!`)
	});
	const bless_reward = await twurple.updateOrCreateCustomReward({
		autoFulfill: true,						//Whether the redemption should automatically set its status to fulfilled.
		backgroundColor: '#FFFF00',				//The hex code of the background color of the reward.
		cost: 777,							//The channel points cost of the reward.
		globalCooldown: null,					//The cooldown between two redemptions of the reward, in seconds. 0 or `null` means no cooldown.
		isEnabled: true,						//Whether the reward is enabled (shown to users).
		maxRedemptionsPerStream: 9999/*first*/,			//The maximum number of redemptions of the reward per stream. 0 or `null` means no limit.
		maxRedemptionsPerUserPerStream: null,	//The maximum number of redemptions of the reward per stream for each user. 0 or `null` means no limit.
		prompt: `Text`,				//The prompt shown to users when redeeming the reward.
		title: `Bless the Run`,				//The title of the reward.
		userInputRequired: false					//Whether the reward requires user input to be redeemed.
	})
	...
	const curse_reward = await twurple.updateOrCreateCustomReward({
		autoFulfill: true,						//Whether the redemption should automatically set its status to fulfilled.
		backgroundColor: '#BD0078',				//The hex code of the background color of the reward.
		cost: 666,							//The channel points cost of the reward.
		globalCooldown: null,					//The cooldown between two redemptions of the reward, in seconds. 0 or `null` means no cooldown.
		isEnabled: true,						//Whether the reward is enabled (shown to users).
		maxRedemptionsPerStream: 9999/*first*/,			//The maximum number of redemptions of the reward per stream. 0 or `null` means no limit.
		maxRedemptionsPerUserPerStream: null,	//The maximum number of redemptions of the reward per stream for each user. 0 or `null` means no limit.
		prompt: `Text`,				//The prompt shown to users when redeeming the reward.
		title: `Curse the Run`,				//The title of the reward.
		userInputRequired: false					//Whether the reward requires user input to be redeemed.
	})
