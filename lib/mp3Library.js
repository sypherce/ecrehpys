...
async function loadMp3Library(directory, force = false) {
	...
	log.debug(`${fileExists ? 'Found' : 'Missing'}`, json_filename);
	if (fileExists) {
...
async function find_new(input) {
	...
	log.debug('b');
	log.debug(`results: ${results.videos[0]} ${JSON.stringify(results.videos[0])}`);
	...
	log.debug(`returnResults: ${returnResults} ${JSON.stringify(returnResults)}`);
