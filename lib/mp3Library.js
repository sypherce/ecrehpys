...
/**
 * Either loads the library for 'directory' from a JSON file, or scans and generates it
 * @param {string} directory Directory containing mp3s
 * @param {boolean} force Rescans all files if true.
 */
...
/**
 * Searches for an mp3 matching "input" ideally formatted as "album -title"
 * @param {string} input Example: "album - title"
 * @param {boolean} force Forces a rescan of all files if true.
 *
 * @return
 */
function find(input) {
	...
	const input_arr = (split_index !== -1) ? [
		input.substring(0, split_index),
		input.substring(split_index + 1)
	] : input;
