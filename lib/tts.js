...
const singingVoiceBag = new ShuffleBag(singingVoices);
...
async function ttsToMP3(string, folder, voice = undefined) {
	...
	let voiceNumber = voices.findIndex((e) => e === voice);
	if (voiceNumber === -1) voiceNumber = allSingingVoices.findIndex((e) => e === voice);

	if (string === '') {
		string = await ermJoke.get();
		server.sayWrapper(`${voice}#${voiceNumber}: ${string}`);
	}
...
module.exports.singing_voices = singingVoices;
module.exports.all_singing_voices = allSingingVoices;
