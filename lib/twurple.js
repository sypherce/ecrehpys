...
function init(streamer_id, streamer_oauth, bot_id, bot_oauth) {
	...
	streamer.onRaid(({ broadcasterName: broadcaster_name, userName: user_name, raidInfo, msg }) => {
		...
		server.sendMessage('Audio', 'assets/alerts/alert_raid.mp3');
	});
...
async function setModerator(user, add_or_remove = true) {
	const user_id = await getUserByName(user);
	const broadcaster_id = await getUserByName(process.env.STREAMER_USER);
	try {
...
async function banUser(data, ban_or_unban = true) {
	console.log(JSON.stringify(data));
	const broadcaster_id = await getUserByName(process.env.STREAMER_USER);
	try {
		if(ban_or_unban === true)
...
async function timeoutUser(data, timeout_or_untimeout = true) {
	if(timeout_or_untimeout === true & parseInt(data.duration) > 0)
	return banUser(data, timeout_or_untimeout);
}

function getChannelInfo() {
	return getChannelInfoById(getUserByName(process.env.STREAMER_USER))
}
...
async function appendTitle(string) {
	...
	if(enabled) {
		...
		await api_client.channels.updateChannelInfo(getUserByName(process.env.STREAMER_USER), {
		//gameId?: string
		//tags?: string[]
		title: `${title} --- ${string}`
		});
