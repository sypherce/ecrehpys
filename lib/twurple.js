...
function init(streamer_id, streamer_oauth, bot_id, bot_oauth) {
	...
	const streamer_auth_provider = new staticAuthProvider(streamer_id, streamer_oauth, [
	...
	const bot_auth_provider = new staticAuthProvider(bot_id, bot_oauth, [
		'channel:manage:moderators',
		'channel:manage:redemptions',
		'channel:read:redemptions',
		'user:read:email',
	...
	eventsub.init(process.env.STREAMER_USER, process.env.STREAMER_ID, process.env.STREAMER_OAUTH, process.env.LISTENER_SECRET, streamer_auth_provider);

	const lurker_counter = [];
		...
		switch (user_name) {
			...
			default:
				if (lurker_counter.indexOf(user_name) === -1) {
					...
					log.debug(`Lurker #${lurker_counter.length}, ${user_name} joined!`);
				}
				break;
		}
	});
		...
		switch (user_name) {
			...
			default:
				if (lurker_counter.indexOf(user_name) !== -1) {
					...
					log.debug(`Lurker #${lurker_counter.length}, ${user_name} parted!`);
				}
				break;
		}
	});
...
async function getChannelInfoByUsername(user_name) {
	const user = (await getUserByName(user_name)) || (await getUserByName(process.env.STREAMER_USER));
	log.debug(user.id);
...
async function setModerator(user, is_add = true) {
	...
	const broadcaster_id = await getUserByName(process.env.STREAMER_USER);
	try {
		if (is_add === true) await api_client.moderation.addModerator(broadcaster_id, user_id);
		else await api_client.moderation.removeModerator(broadcaster_id, user_id);
	} catch (error) {
		console.error(error);
	}
...
async function banUser(data, is_ban = true) {
	...
	const broadcaster_id = await getUserByName(process.env.STREAMER_USER);
	try {
		if (is_ban === true) await api_client.moderation.banUser(broadcaster_id, data);
		else await api_client.moderation.unbanUser(broadcaster_id, data.user);
	} catch (error) {
		console.error(error);
	}
...
async function timeoutUser(data, is_timeout = true) {
	if ((is_timeout === true) & (parseInt(data.duration) > 0)) return banUser(data, is_timeout);
}
...
async function createCustomReward(
	...
	try {
		...
		const reward = api_client.channelPoints.createCustomReward(broadcaster_id, data);
		log.debug(reward.id);
		return reward;
	} catch (error) {
		log.debug(error);
...
async function getCustomRewards(onlyManageable = true) {
	const broadcaster_id = await getUserByName(process.env.STREAMER_USER);
	return await api_client.channelPoints.getCustomRewards(broadcaster_id, onlyManageable);
}
